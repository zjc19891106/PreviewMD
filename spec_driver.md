...
无论使用 Spec Kit 还是 OpenSpec，AI 生成质量很大程度上取决于你提供的上下文是否充足。
...
| 上下文类型            | 说明                                                 |
| --------------------- | ---------------------------------------------------- |
| 已有工具类 / 基础设施 | 告知 AI 项目中已有哪些可复用的工具类、Helper、抽象层 |
| 相似实现的代码示例    | 让 AI 读取已有的相似实现，作为参考风格和模式         |
| 编码规范 / 项目约定   | 命名规范、目录结构、错误处理方式等                   |
| 接口定义 / 数据结构   | 给出相关的类型定义、接口签名、数据模型               |
| 实现方式的明确指导    | 直接告诉 AI “用 XXX 方式实现”，而不是让它自己挑选  |
...
| 方法               | 说明                                                           |
| ------------------ | -------------------------------------------------------------- |
| 直接给出代码片段   | 在 Prompt 中粘贴已有的工具类、接口定义等                       |
| 让 AI 读取指定文件 | 告诉 AI 去读取某个已有实现文件，作为参考                       |
| 让 AI 提炼上下文   | 要求 AI 先分析项目结构、总结现有模式，再开始实现               |
| 文档化的规范文件   | 将项目约定写入`AGENTS.md` 或 `constitution` 中，让 AI 自动遵守 |

#### 新功能的测试保障

- 新加 feature 应要求 AI 同时输出测试用例，确保实现有测试覆盖
- 测试可以包含：单元测试、集成测试、边界场景
- 在 `tasks.md` 中明确列出测试任务，作为 DoD 的一部分
